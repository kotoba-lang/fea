...
;; kotoba-lang/fea 用に com-junkawasaki/root superproject の manifest/edn-datomize.bb
;; から移植（Phase 3 fan-out, "EDN datomize" ワークストリーム）。schema-path を
;; この repo のルート schema.edn に向けている以外はロジック同一 ＋ wrap-vec
;; モードを追加（トップレベルが vector-of-maps のファイル用 — 各要素を multi-entity
;; tx-data の1 entity にする。materials.edn がこの形）。
;;
...
;;   bb scripts/edn-datomize.bb wrap-vec <path> <ns>     — vector-of-maps ファイルを変換（各要素→1 entity）
...
;; fea (child repo) has no manifest/ dir — schema lives at repo root.
...
;; ---------- vector-of-maps (top-level vector, each element a bare-key map —
;; e.g. materials.edn's [{:name ... :model {...}} ...]). Unlike wrap-map!
;; (1 file -> 1 entity), each vector element becomes its own entity
;; (:db/id -1, -2, -3, ...), all sharing the same ns-name prefix. ----------

(defn entity-from-map-at [content ns-name idx]
  (into {:db/id (- (inc idx))}
        (map (fn [[k v]] [(namespaced-key ns-name k) (attr-value v)]))
        content))
...
(defn wrap-vec! [rel-path ns-name]
  (let [f (io/file root rel-path)
    ...
    (cond
      ...
      (println "skip (not a non-empty vector-of-maps):" rel-path)

      :else
      (let [entities (into [] (map-indexed (fn [i m] (entity-from-map-at m ns-name i)) content))
            attrs (mapcat #(schema-attrs % ns-name) content)]
        (spit f (str cmt (pr-str entities) "\n"))
        (merge-schema! attrs)
        (println "wrapped(vec)" rel-path "->" (count entities) "entities, ns=" ns-name)))))
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file"     (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                       ...
                       (merge-schema! (:attrs @report))
                       (println @report))
      (do (println "usage: bb scripts/edn-datomize.bb [wrap-map <path> <ns> | wrap-vec <path> <ns> | wrap-generic <path> <default-ns> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
